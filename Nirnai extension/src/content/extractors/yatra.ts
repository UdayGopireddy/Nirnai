// Yatra hotel/property page extractor
...
registerSearchUrlBuilder("yatra", (p: CrossSiteSearchParams): string => {
  const sp = new URLSearchParams();
  if (p.destination) sp.set("searchCriteria", p.destination);
  if (p.checkin) sp.set("checkIn", p.checkin);
  if (p.checkout) sp.set("checkOut", p.checkout);
  if (p.adults) sp.set("paxInfoStr", `${p.adults}-0-0`);
  if (p.rooms) sp.set("roomCount", p.rooms || "1");
...
export class YatraExtractor implements SiteExtractor {
  siteName(): string { return "yatra"; }

  isProductPage(): boolean {
    if (window.location.pathname.includes("/hotels/hotel-")) return true;
    if (/\/hotels\/[^/]+\/[^/]+-\d+/.test(window.location.pathname)) return true;
    if (document.querySelector('[class*="hotel-detail"], #hotelDetailPage')) return true;
    return false;
  }
  ...
  isSearchPage(): boolean {
    ...
    if (window.location.pathname.includes("/hotels/result")) return true;
    if (document.querySelector('[class*="hotel-list"], [class*="hotelTuple"]')) return true;
    return false;
  }
  ...
  extractSearchListings(maxResults: number = 20): ProductData[] {
    ...
    if (cards.length === 0) {
      ...
      for (const link of links) {
        ...
        if (seen.has(key) || href.includes("/search")) continue;
        seen.add(key);
        const container = link.closest('[class*="card"], [class*="hotel"], [class*="tuple"], li, article') || link.parentElement?.parentElement;
    ...
    for (const card of cards.slice(0, maxResults)) {
      try {
        ...
        const linkEl = (card.querySelector('a[href*="/hotels/hotel-"]') || card.querySelector('a')) as HTMLAnchorElement | null;
        const url = linkEl ? new URL(linkEl.href, window.location.origin).href : "";
        let price = card.querySelector('[class*="Price"], [class*="price"], [class*="tariff"]')?.textContent?.trim() || "";
        if (!price) {
  ...
  getSearchParams(): CrossSiteSearchParams | null {
    ...
    const destination = params.get("searchCriteria") || params.get("city") || getMeta("og:title")?.replace(/ Hotels.*/i, "") || "";
    if (!destination) return null;
    return {
      destination,
      checkin: params.get("checkIn") || "",
      checkout: params.get("checkOut") || "",
      adults: params.get("paxInfoStr")?.split("-")[0] || "",
      children: "",
  ...
  getSearchDestination(): string {
    const params = new URLSearchParams(window.location.search);
    return params.get("searchCriteria") || params.get("city") || getMeta("og:title")?.replace(/ Hotels.*/i, "") || "";
  }

  extractProduct(): ProductData {
    const jsonLd = getJsonLd();
    const title = getMeta("og:title")?.replace(/ - Yatra$/i, "") || jsonLd?.name || extractText(['h1', '[class*="hotel-name"]']);
    let price = jsonLd?.offers?.price?.toString() || "";
    if (!price) price = extractText(['[class*="Price"], [class*="price"], [class*="tariff"]']);
    ...
    return {
      ...
      barcode: "", source_site: "yatra", page_type: "product",
    };
  }
