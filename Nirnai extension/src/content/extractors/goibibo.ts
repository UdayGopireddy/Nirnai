// Goibibo hotel/property page extractor
...
registerSearchUrlBuilder("goibibo", (p: CrossSiteSearchParams): string => {
  const slug = p.destination.replace(/\s+/g, "-");
  const sp = new URLSearchParams();
  if (p.checkin) sp.set("checkin", p.checkin);
  if (p.checkout) sp.set("checkout", p.checkout);
  if (p.adults) sp.set("guests", p.adults);
...
export class GoibiboExtractor implements SiteExtractor {
  siteName(): string { return "goibibo"; }

  isProductPage(): boolean {
    if (/\/hotels\/[^/]+-\d+/.test(window.location.pathname)) return true;
    if (window.location.pathname.includes("/hotel-details")) return true;
    if (document.querySelector('[class*="hotel-detail"], [data-testid="hotel-detail"]')) return true;
    return false;
  }
  ...
  isSearchPage(): boolean {
    ...
    if (document.querySelector('[data-testid="hotel-card"], [class*="HotelCardstyles"]')) return true;
    return false;
  }
  ...
  extractSearchListings(maxResults: number = 20): ProductData[] {
    ...
    if (cards.length === 0) {
      const links = document.querySelectorAll('a[href*="/hotels/"], a[href*="/hotel-details"]');
      const seen = new Set<string>();
      for (const link of links) {
        const href = (link as HTMLAnchorElement).href;
        const key = href.split("?")[0];
        if (seen.has(key) || href.includes("/hotels-in-")) continue;
        seen.add(key);
    ...
    for (const card of cards.slice(0, maxResults)) {
      try {
        ...
        const titleEl = card.querySelector('h3, h2, [class*="hotel-name"], [class*="hotelName"], [data-testid="hotel-name"]');
  ...
  getSearchParams(): CrossSiteSearchParams | null {
    ...
    // Extract city from URL path like /hotels-in-Goa-ct/
    const pathMatch = window.location.pathname.match(/\/hotels-in-([^/-]+)/);
    const destination = pathMatch ? pathMatch[1].replace(/-/g, " ") : params.get("city") || getMeta("og:title")?.replace(/ Hotels.*/i, "") || "";
    if (!destination) return null;
    return {
      destination,
      checkin: params.get("checkin") || "",
      checkout: params.get("checkout") || "",
      adults: params.get("guests") || params.get("adults") || "",
  ...
  getSearchDestination(): string {
    const params = new URLSearchParams(window.location.search);
    const pathMatch = window.location.pathname.match(/\/hotels-in-([^/-]+)/);
    return pathMatch ? pathMatch[1].replace(/-/g, " ") : params.get("city") || getMeta("og:title")?.replace(/ Hotels.*/i, "") || "";
  }

  extractProduct(): ProductData {
    const jsonLd = getJsonLd();
    const title = getMeta("og:title")?.replace(/ - Goibibo$/i, "") || jsonLd?.name || extractText(['h1', '[class*="hotel-name"]']);
    ...
    return {
      ...
      barcode: "", source_site: "goibibo", page_type: "product",
    };
  }
