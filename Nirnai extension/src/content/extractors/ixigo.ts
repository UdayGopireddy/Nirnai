// Ixigo hotel/property page extractor
...
registerSearchUrlBuilder("ixigo", (p: CrossSiteSearchParams): string => {
...
export class IxigoExtractor implements SiteExtractor {
  siteName(): string { return "ixigo"; }
  ...
  isSearchPage(): boolean {
    ...
    if (window.location.pathname.includes("/search/result/hotel")) return true;
  ...
  extractSearchListings(maxResults: number = 20): ProductData[] {
    ...
    if (cards.length === 0) {
      const links = document.querySelectorAll('a[href*="/hotels/"]');
  ...
  getSearchParams(): CrossSiteSearchParams | null {
    ...
    const destination = params.get("city") || params.get("searchFor") || getMeta("og:title")?.replace(/ Hotels.*/i, "") || "";
  ...
  getSearchDestination(): string {
    const params = new URLSearchParams(window.location.search);
    return params.get("city") || params.get("searchFor") || getMeta("og:title")?.replace(/ Hotels.*/i, "") || "";
  }

  extractProduct(): ProductData {
    const jsonLd = getJsonLd();
    const title = getMeta("og:title")?.replace(/ - ixigo$/i, "") || jsonLd?.name || extractText(['h1', '[class*="hotel-name"]']);
    ...
    return {
      ...
      barcode: "", source_site: "ixigo", page_type: "product",
    };
  }
