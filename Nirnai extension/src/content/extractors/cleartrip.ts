// Cleartrip hotel/property page extractor
...
registerSearchUrlBuilder("cleartrip", (p: CrossSiteSearchParams): string => {
  const sp = new URLSearchParams();
  if (p.destination) sp.set("city", p.destination);
  if (p.checkin) sp.set("chk_in", p.checkin);
  if (p.checkout) sp.set("chk_out", p.checkout);
  if (p.adults) sp.set("adults", p.adults);
  if (p.children) sp.set("childs", p.children);
  if (p.rooms) sp.set("num_rooms", p.rooms || "1");
...
export class CleartripExtractor implements SiteExtractor {
  siteName(): string { return "cleartrip"; }

  isProductPage(): boolean {
    if (window.location.pathname.includes("/hotel/") && /\/\d+/.test(window.location.pathname)) return true;
  ...
  isSearchPage(): boolean {
    ...
    if (window.location.pathname.includes("/hotels/results")) return true;
    if (window.location.pathname.includes("/hotels/search")) return true;
    if (document.querySelector('[class*="hotel-card"], [data-testid="hotel-card"]')) return true;
    return false;
  }
  ...
  extractSearchListings(maxResults: number = 20): ProductData[] {
    ...
    for (const card of cards.slice(0, maxResults)) {
      try {
        ...
        const linkEl = (card.querySelector('a[href*="/hotel/"]') || card.querySelector('a')) as HTMLAnchorElement | null;
  ...
  getSearchParams(): CrossSiteSearchParams | null {
    ...
    return {
      destination,
      checkin: params.get("chk_in") || params.get("checkin") || "",
      checkout: params.get("chk_out") || params.get("checkout") || "",
      adults: params.get("adults") || "",
      children: params.get("childs") || params.get("children") || "",
      rooms: params.get("num_rooms") || "1",
      query: "",
    };
  }
  ...
  extractProduct(): ProductData {
    ...
    const title = getMeta("og:title")?.replace(/ - Cleartrip$/i, "") || jsonLd?.name || extractText(['h1', '[class*="hotel-name"]']);
    ...
    return {
      ...
      barcode: "", source_site: "cleartrip", page_type: "product",
    };
  }
